...
def index():
    ...
    if 'username' in session:
        ...
        return render_template('legacy_home.html', token=session['token'])
        #return 'Logged in as %s' % session['username']
    # Redirct the user to the signup page
    return redirect(url_for('legacy_signup'))
...
def signup():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('legacy_index'))
    return render_template('legacy_signup.html'), 401
...
def processCommand(command, username, channelId, permissionLevel):
    ...
    if command_name == 'logout':
        return redirect(url_for('legacy_logout'))
        ...
        for user in users:
            if user == username:
                ...
                if username == session['username']:
                    return render_template('legacy_profile.html', username=username, permissionLevel=users[username].get('permissionLevel', -1), 
...
def require_auth():
    ...
    return render_template('legacy_unauthorized.html')
